...
-   属性->源, 查看它的范围, 投影, 等
-   打开属性表. 
-   可以发现辽宁省的很多小岛都被算成单独的要素, 要把它合并. 地理处理->融合, 选择要融合的字段. 这里我们选择`NAME`. **注意融合的时候不能选中任何一个对象**, 如果已经选择了, 要用选择工具在空白的地方点一下以取消选择, 否则只会对选择的那个对象进行处理. 
-   对属性表进行操作. 
...
全自动矢量化: 效果不是特别好, 尤其是后期处理比较麻烦, 除非原式图象质量非常好. 
...
-   `.dbf`用来存放属性信息, 但是可以用Excel打开. 这样就可以利用Excel强大的数据分析功能. **注意分析过程中不要改动`.dbf`文件的结构** (比如改动行列的顺序, 增加/删除行列, 改字段名, etc), 处理之前最好做一个备份, 否则可能造成文件损坏而打不开. 
